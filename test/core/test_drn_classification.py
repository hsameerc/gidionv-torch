...
language_alphabets = {"English": set("abcdefghijklmnopqrstuvwxyz"),
    "French": set("abcdefghijklmnopqrstuvwxyzàâæçéèêëîïôœùûüÿ"), "German": set("abcdefghijklmnopqrstuvwxyzäöüß"),
    "Spanish": set("abcdefghijklmnopqrstuvwxyzáéíñóúü"), "Russian": set("абвгдеёжзийклмнопрстуфхцчшщъыьэюя")}
...
examples = {lang: [(encode_text_numpy(sent), one_hot_numpy(class_labels.index(lang))) for sent in sents] for lang, sents
    in {"English": ["Hello world", "The cat jumps", "Good morning", "I love coding", "She is reading a book",
        "It is raining outside", "We are going home", "They play football", "This is my dog", "Can you help me?"],
        "French": ["Bonjour le monde", "Je t'aime", "Il fait beau", "J'adore le chocolat", "Elle lit un livre",
            "Il pleut dehors", "Nous rentrons à la maison", "Ils jouent au football", "Ceci est mon chien",
        ...
        "Spanish": ["Hola mundo", "Te amo", "Buenos días", "Me gusta el chocolate", "Ella está leyendo un libro",
            "Está lloviendo afuera", "Vamos a casa", "Juegan al fútbol", "Este es mi perro", "¿Puedes ayudarme?"],
...
class TestTorchDynamicFeedForwardNetwork(unittest.TestCase):
    ...
    def test_french(self):
        self._test_prediction("Bonjour le monde", "French")
    ...
    def test_forward_speed(self):
        ...
        print(f"Average PyTorch forward pass latency over batch of 1000: {avg_ms:.2f} ms")
        self.assertLess(avg_ms, 50, "Forward pass is too slow")  # PyTorch should be much faster!

    def test_memory_leak(self):
        # This test becomes less relevant as PyTorch handles memory, but can still be run.
        self.net.eval()
        with torch.no_grad():
            x = torch.randn(1000, input_size, dtype=torch.float32, device=self.device)
            for _ in range(100):
                _ = self.net(x)
        # Note: Measuring memory on GPU is different. `torch.cuda.memory_allocated()` is better.
        print("Skipping memory leak test as it requires specific GPU memory tools for PyTorch.")

    def test_empty_input(self):
        self.net.eval()
        with torch.no_grad():
            x = torch.zeros((1, input_size), dtype=torch.float32, device=self.device)
            logits = self.net(x)
            self.assertFalse(torch.isnan(logits).any(), "NaNs in output for empty input")
