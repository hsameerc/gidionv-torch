...
class MultiMemoryTransformer(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, memory_streams_ids: Optional[List[torch.Tensor]] = None,
        ...
        for i, block in enumerate(self.decoder_blocks):
            ...
            # print(f"DEBUG After Decoder Block {i}: x.abs().max() = {x.abs().max().item()}")
            next_kv_caches.append(updated_kv_cache)
        # Final Layers
        x = self.final_norm(x)
        # print(f"DEBUG After Final Norm: x.abs().max() = {x.abs().max().item()}")
        logits = self.lm_head(x)
        # print(f"DEBUG After LM Head: logits.abs().max() = {logits.abs().max().item()}")
