...
class LIF_Layer(nn.Module):
    def __init__(self, input_size: int, hidden_size: int, refractory_steps: int = 3,
        ...
        self.linear_in = nn.Linear(input_size, hidden_size, dtype=dtype)
    ...
    def forward(self, x_t, state, return_spike=False):
        # Unpack the state from the previous timestep
        ...
        # Now, determine which neurons actually fired in the last step to decide if they need a reset
...
class LIF_RNN(nn.Module):
    ...
    def __init__(self, input_size: int, output_size: int, hidden_layers_config: List[int], **kwargs):
        ...
        for hidden_size in hidden_layers_config:
            self.lif_layers.append(LIF_Layer(layer_input_size, hidden_size, **kwargs))
